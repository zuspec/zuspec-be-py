...
so the version can live in src/zuspec/be/py/__version__.py and reach setuptools
...
captures the dotted attribute path and leaves the closing brace stranded:
    ...
    version="zuspec.be.py.__version__.VERSION.dev<run-id>+gh"}
...
def _get_version():
    version_file = os.path.join(proj_dir, "src", "zuspec", "be", "py", "__version__.py")
    glb = {}
