...
# NOTE: ActivityParser / ActivityAnonTraversal / ActivitySequenceBlock are the
# Python-source `activity()` parser — a dataclasses *frontend* concern.  Imported
# lazily (function-local) so this backend module does not create a be.py → dataclasses
# import cycle.  The IR-driven (pssc) path sets __activity__ from IR and never reaches
# the parse below; only the @zdc-authored path hits it.
...
class ScenarioCompiler:
    ...
    def compile(
        ...
        # This fast-path parses a ``@zdc``-authored Python ``activity()`` method via
        # the dataclasses ActivityParser.  IR-built actions (the pssc path) instead
        # carry their activity as IR in ``__activity__`` and are handled by the
        # generic activity runner — skip the fast-path (and avoid importing the
        # dataclasses frontend parser at all).  Detected by module name so no
        # ``zuspec.dataclasses`` import is needed.
        _act_ir = getattr(action_type, "__activity__", None)
        if _act_ir is not None and type(_act_ir).__module__.startswith("zuspec.ir.core"):
            return None
        ...
        from zuspec.dataclasses.activity_parser import ActivityParser, ActivitySequenceBlock
        try:
...
def _is_flat_sequence(ir: "ActivitySequenceBlock") -> bool:
    ...
    from zuspec.dataclasses.activity_parser import ActivityAnonTraversal
