...
action→owning-component map.  Any IR producer — not just PSS — can drive it.
"""
...
class ClassRegistry:
    ...
    def __init__(self, classes: Dict[str, Any],
        ...
        # action qualified-name -> owning component name (for root selection)
        object.__setattr__(self, '_owner_map', dict(owner_map or {}))
        # The IR type map this registry was built from (for emit_module / IR
        # serialization); shared node objects, not a copy.
        object.__setattr__(self, '_type_m', type_m or {})
        # export-api configuration (populated by build_registry / the frontend)
        object.__setattr__(self, '_export_actions', [])
        object.__setattr__(self, '_import_specs', {})

    # --- export-api configuration --------------------------------------------

    def configure_exports(self, export_actions=None, import_specs=None) -> "ClassRegistry":
        """Set the exported actions and import specs used by :meth:`create`."""
        object.__setattr__(self, '_export_actions', list(export_actions or []))
        object.__setattr__(self, '_import_specs', dict(import_specs or {}))
        return self
    ...
    @property
    def export_actions(self) -> List[str]:
        return self._export_actions

    @property
    def import_specs(self) -> Dict[str, Any]:
        return self._import_specs

    def action_qnames(self) -> List[str]:
        """All built action types, by qualified name (``'Comp::Action'``)."""
        return [
            k for k, v in self._classes.items()
            if isinstance(v, type) and issubclass(v, zdc.Action)
            and '::' in k
        ]

    def root_component_name(self, export_actions: Optional[List[str]] = None) -> str:
        """Name of the component to instantiate as the run's root.

        Prefers the component owning the exported action(s); falls back to the
        sole component when the model defines exactly one.
        """
        acts = export_actions or self._export_actions or self.action_qnames()
        for q in acts:
            owner = self._owner_map.get(q)
            if owner is not None:
                return owner
        comps = [k for k, v in self._classes.items()
                 if isinstance(v, type) and issubclass(v, zdc.Component)
                 and '::' not in k]
        if len(comps) == 1:
            return comps[0]
        raise ValueError(
            "cannot determine a root component to instantiate; pass root=... to "
            f"create() (candidates: {sorted(comps)})")

    # --- runnable export-api factory -----------------------------------------

    def create(self, imports=None, *, seed=None, root=None):
        """Build a runnable per-run :class:`ExportApi` instance.

        Instantiates a fresh component tree, wires *imports* through an import
        resolver, and exposes one async method per exported action.  Call again
        for an independent run (new tree, new seed).
        """
        from .export_api import ExportApi
        return ExportApi._build(self, imports=imports, seed=seed, root=root)

    @property
    def ImportApi(self):
        """A structural stub class naming the imports this model requires."""
        from .export_api import build_import_protocol
        return build_import_protocol(self._import_specs)

    def emit_module(self, path=None, *, module_doc: Optional[str] = None) -> str:
        """Emit a self-contained Python module that reconstructs this registry.

        The generated module embeds the model's serialized IR plus the export
        configuration and rebuilds the registry on import — no PSS re-parse.
        Returns the source; also writes it to *path* when given.  Import the
        module and drive ``module.registry`` exactly like a ``load_pss(...)`` one.
        """
        from .export_api import render_module
        src = render_module(self, module_doc=module_doc)
        if path is not None:
            import pathlib
...
class IrToRuntimeBuilder:
    """Convert an IR ``Context`` into a dict of Python runtime classes.
    ...
    *ctx* is a :class:`zuspec.ir.core.Context` (its ``type_m`` map is consumed; a
    legacy object exposing ``type_map`` is tolerated during the migration).
    *owner_map* maps each qualified action name (``'MyC::MyA'``) to its owning
    component name (``'MyC'``); when omitted it is derived from the IR.
    ...
    def __init__(self, ctx, *, owner_map: Optional[Dict[str, str]] = None,
                 import_names: Optional[set] = None):
        self.ctx = ctx
        # Canonical IR: prefer the core Context.type_m; tolerate a legacy
        # AstToIrContext.type_map during the migration.
        type_m = getattr(ctx, 'type_m', None)
        if type_m is None:
            type_m = getattr(ctx, 'type_map', None)
        if type_m is None:
            raise TypeError(
                "IrToRuntimeBuilder requires a Context with 'type_m' "
                "(or legacy 'type_map')")
        self._type_m: Dict[str, Any] = type_m
        # action qualified-name -> owning component name.  An explicit map wins;
        # otherwise derive it from the IR so the builder stays independent of any
        # PSS-specific side table.
        self._owner_map: Dict[str, str] = (
            owner_map if owner_map is not None else self._derive_owner_map())
        # Package-scope import names — bodies referencing one are left to the
        # interpreter (so the import resolver routes the call).  Explicit set
        # wins; otherwise derive from the PSS context's import declarations.
        if import_names is not None:
            self._import_names = set(import_names)
        else:
            self._import_names = {
                f.name for f in getattr(ctx, 'import_functions', [])
            }
        # Accumulates built Python classes; populated during build()
        self.python_classes: Dict[str, Any] = {}

    def _derive_owner_map(self) -> Dict[str, str]:
        """Recover action→component ownership from the IR alone.

        For each qualified ``Comp::Action`` key whose value is an action
        (``DataTypeClass`` but not ``DataTypeComponent``) and whose prefix names
        a ``DataTypeComponent`` in the type map, the owner is that prefix.
        Namespace-/package-qualified actions (prefix is not a component) have no
        owner and remain top-level.
        """
        owner: Dict[str, str] = {}
        for k, v in self._type_m.items():
            if '::' not in k:
                continue
            if (not isinstance(v, zdc_ir.DataTypeClass)
                    or isinstance(v, zdc_ir.DataTypeComponent)):
                continue
            prefix = k.rsplit('::', 1)[0]
            if isinstance(self._type_m.get(prefix), zdc_ir.DataTypeComponent):
                owner[k] = prefix
        return owner
    ...
    def build(self) -> ClassRegistry:
        ...
        for name, dt in self._type_m.items():
        ...
        struct_entries = [
            (k, v) for k, v in self._type_m.items()
        ...
        for qname, dt in action_entries:
            parent_name = self._owner_map.get(qname)
        ...
        return ClassRegistry(self.python_classes, owner_map=self._owner_map,
                             type_m=self._type_m)
    ...
    def _build_body_fn(self, func: zdc_ir.Function):
        ...
        if compiled is not None:
            async def body(self_action):
                compiled(self_action)
            return body

        async def body(self_action):
            try:
                # Async executor: awaits async `import target` calls; falls back
                # to synchronous execution for any import-free statement.
                await AsyncObjectExecutor(self_action).execute_stmts_async(stmts)
    ...
    def _array_element_class(self, elem_dt) -> Optional[type]:
        """Resolve the built Python class for an array's element type, or None
        for scalar (int/enum/...) elements. Covers struct/class/component element
        types (referenced inline as DataTypeStruct, or by DataTypeRef)."""
        elem_cls = None
        if isinstance(elem_dt, zdc_ir.DataTypeRef):
            elem_cls = self.python_classes.get(elem_dt.ref_name)
            if elem_cls is None:
                for qname, py_cls in self.python_classes.items():
                    if qname.split('::')[-1] == elem_dt.ref_name:
                        elem_cls = py_cls
                        break
        elif isinstance(elem_dt, zdc_ir.DataTypeStruct) and getattr(elem_dt, 'name', None):
    ...
    def _field_to_plain(self, f: zdc_ir.Field) -> Tuple[Any, Any]:
        ...
        if isinstance(dt, zdc_ir.DataTypeInt):
            ...
            # Array of a struct type: default each element to a fresh instance so
            # member fields (e.g. pkts[i].x) exist for randomization/writeback.
            elem_cls = self._array_element_class(dt.element_type)
            if elem_cls is not None:
                return list, _stdlib_dc.field(
                    default_factory=lambda cls=elem_cls, _n=n: [cls() for _ in range(_n)])
    ...
    def _field_to_zdc(self, f: zdc_ir.Field) -> Tuple[Any, Any]:
        ...
        if isinstance(dt, zdc_ir.DataTypeInt):
            ...
            n = dt.size if dt.size > 0 else 0
            # Array of a struct/component type: build a list of fresh instances
            # so member fields exist (for randomization, writeback, traversal).
            elem_cls = self._array_element_class(dt.element_type)
            if elem_cls is not None:
    ...
    def _resolve_field_types(self, dt: zdc_ir.DataTypeStruct):
        ...
        for f in dt.fields:
            if isinstance(f.datatype, zdc_ir.DataTypeRef):
                resolved = self._type_m.get(f.datatype.ref_name)
    ...
    def _topo_sort(
        ...
        def visit(name: str):
            ...
            for f in dt.fields:
                ft = f.datatype
                # Unwrap an array to its element type for dependency purposes.
                if isinstance(ft, zdc_ir.DataTypeArray):
                    ft = ft.element_type
                if isinstance(ft, zdc_ir.DataTypeRef):
                    visit(ft.ref_name)
                elif isinstance(ft, zdc_ir.DataTypeStruct) and getattr(ft, 'name', None):
                    visit(ft.name)
...
def build_runtime(ctx, *, owner_map: Optional[Dict[str, str]] = None,
                  import_names: Optional[set] = None) -> ClassRegistry:
    """Build a :class:`ClassRegistry` of live Python classes from an IR ``Context``.

    Thin wrapper over :class:`IrToRuntimeBuilder` — the single entry point for
    turning a :class:`zuspec.ir.core.Context` into runnable ``zdc`` classes.
    """
    return IrToRuntimeBuilder(
        ctx, owner_map=owner_map, import_names=import_names).build()
